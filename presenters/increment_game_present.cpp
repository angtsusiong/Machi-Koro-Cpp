#include "increment_game_present.h"
...
IncrementGamePresenter::IncrementGamePresenter(std::shared_ptr<UtilBase> util)
    : util_(util)
    , view_model_(std::make_unique<ViewModel>())
{
}

void IncrementGamePresenter::present(Event* event)
{
    auto res = dynamic_cast<IncrementGameEvent*>(event);
...
IncrementGamePresenter::ViewModel::ViewModel(std::shared_ptr<UtilBase> util,
...
IncrementGamePresenter::ViewModel::~ViewModel()
{
    bank_ = nullptr;
    market_ = nullptr;
    players_ = nullptr;
...
Json::Value IncrementGamePresenter::ViewModel::getJson() const
{
    Json::Value res;
